...
这条路线把 **「头文件版本可控」**（vendor rdma-core）与 **「bindgen 生成 `extern "C"`」** 结合起来，但对 **`verbs.h` 里的 `static inline` 热点**采用了与其它 `-sys` crate **不同的分工**：**不在 `ibverbs-sys` 里用一整份 `verbs.rs` 复述 inline**，而是把 **`context.ops` 上的函数指针调用**写进 **`ibverbs` 安全封装层**的方法实现里。

因此：**FFI 架构上是「生成绑定 + 手写派发」，但手写派发发生的层级是 `ibverbs`（safe），而不是 `ibverbs-sys`。**
...
```text
                    ┌─────────────────────────────────────┐
                    │           ibverbs (safe crate)       │
                    │  QueuePair::post_send / Device / …   │
                    │  内部：unsafe { ops.post_send(...)}   │  ← inline 语义在此层落地
                    └─────────────────┬───────────────────┘
                                      │ 调用 ibverbs-sys 类型与部分符号
                                      ▼
                    ┌─────────────────────────────────────┐
                    │        ibverbs-sys ( -sys )          │
                    │  include!(OUT_DIR/bindings.rs)       │  ← bindgen；无完整 ibv_post_send 体
                    │  + blocklist 类型的手写补全           │
                    └─────────────────┬───────────────────┘
                                      │ extern "C" + 链接
                                      ▼
                         系统 libibverbs.so（运行时）
```

**关键语义：** C 头文件里的 `ibv_post_send(qp, …)` 等价于「从 `qp` 找到 `ibv_context`，再调 `ops.post_send`」。bindgen 通常不给这条链路生成可调用的 Rust 函数体；本方案 **在 safe API 的实现里直接走完这条链路**，于是 **`ibverbs-sys` 更像「类型与非内联符号的底座」，而不是「verbs 大全」。**
...
| 边界 | 落在何处 | 说明 |
|------|----------|------|
| **C ABI 类型与枚举** | `ibverbs-sys`（生成 + 少量手写） | `#[repr(C)]` 布局必须与 rdma-core 一致 |
| **`static inline` 数据路径** | **`ibverbs`** | 通过 `ops` 指针间接调用，与 `verbs.h` 语义对齐 |
| **链接对象** | 动态 **`libibverbs`** | vendor/cmake 主要用于 **头文件路径稳定**，不是把 verbs 静态打进一个巨型 `.a`（具体以该 crate 文档为准） |

**设计取舍简述：** `-sys` 保持「薄」——生成脚本简单、重复代码少；代价是 **仅依赖 `ibverbs-sys` 的用户若要调用 post_send/poll_cq 语义，必须自己理解并实现 ops 派发**，心智负担转移到上层或与 `ibverbs` 一并使用。

---

## 编译流水线（概念）

1. **cmake / vendor：** 配置 `vendor/rdma-core`，得到一致的 `-I` 与 `verbs.h` 视图（也可用环境变量指向外部 rdma-core）。
2. **bindgen：** 对头文件做 allowlist/blocklist（如对难生成的 `ibv_wc` blocklist），产出 **`$OUT_DIR/bindings.rs`**。
3. **`include!`：** `ibverbs-sys` 在编译期合并生成码与手写补全。
4. **链接：** `rustc-link-lib=ibverbs`，运行时加载系统 **`libibverbs.so`**。

---

## 安全 Rust 封装层：`ibverbs`

**职责：** 在 **`ibverbs-sys`** 提供的原始指针与类型之上，封装 **Device / QueuePair / CompletionQueue** 等资源，提供 **更符合 Rust 习惯的所有权与 API**，并在 **`post_send` 等方法内部** 执行 **`unsafe` 块中的 ops 派发**（例如从 `self.qp` 取 `context`、`ops.post_send.unwrap()(…)`）。

**与安全模型的关系：** 外层 API 尽量 **safe**，但底层仍是 **RDMA 固有的不安全契约**（内存注册、远程访问），因此封装通常是 **「局部 unsafe 收缩 + 类型状态机」**，而非宣称覆盖全部 UB 场景。

**与 DatenLord 对比：** DatenLord 把「等价于 inline 的 `unsafe fn`」放进 **`rdma-sys`**；jonhoo 把同一逻辑放进 **`ibverbs`**。**两层 crate 都存在**，只是 **FFI 完整性在哪一层「闭合」**不同。
...
本项目的做法是：**在 safe `ibverbs` crate 中手写等价逻辑**，例如从 `qp` 取出 `context`、`ops`，再调用 `ops.post_send.unwrap()(qp, ...)`（见工作区 `rdma/rust-ibverbs/ibverbs/src/lib.rs` 中 `post_send`）。
...
**最广为人知的 ibverbs 教学型栈之一**；FFI 上属于 **bindgen + vendor 头文件 + 安全层函数指针派发**。与「在 `-sys` 写一整份手写 inline 包装」（datenlord）**分层相同（sys + safe），但闭合 FFI 热点的层级不同**。
