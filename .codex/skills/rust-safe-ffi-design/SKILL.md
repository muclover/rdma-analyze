...
| **位置** | 默认 `<输出根>/rust-ffi-design/`；能从上下文推断则直接使用，缺失时一次性询问 |
...
Reference 是内部决策材料，不是交付物内容。读取要按需、分层，避免把样本痕迹带入最终文档。
...
2. [references/guide/architecture-decisions.md](references/guide/architecture-decisions.md) — Q1-Q9、crate 分层和绑定决策树。
3. [references/guide/rust-ffi-best-practices.md](references/guide/rust-ffi-best-practices.md) — safe 边界、反模式、资源/错误原则。
4. [references/guide/new-project-checklist.md](references/guide/new-project-checklist.md) — `CHECKLIST.md` 的覆盖基线。
...
| 画像触发 | 可读 reference |
|----------|----------------|
| 纯 `-sys`、复杂 `build.rs` / system-vendored 切换 | `references/libz-sys/FFI-ANALYSIS.md` |
| 双 crate、回调、panic/线程边界 | `references/curl-rust/FFI-ANALYSIS.md` |
| workspace、错误栈、手写+bindgen 混合 | `references/rust-openssl/FFI-ANALYSIS.md` |
| 三层、预生成 bindgen、大 API 面 | `references/zstd-rs/FFI-ANALYSIS.md` |
| ibverbs / rdma-core / RDMA 相关 | `references/rdma/README.md` 与 `references/rdma/rdma-overview.md` |
| 需要横向模式核对 | `references/comparison/general-c-ffi.md` |

### 禁止泄漏

交付物中不要出现：

- `P1`-`P11`、`FFI-ANALYSIS`、`general-c-ffi`、`comparison`、`→ §x` 等内部证据标记。
- 「样本 P4」「参考某项目」这类样本归因。
- 大段复制 reference 原文。
...
| **输出根目录** | 用户指定优先；否则默认 C 项目同级或 `docs/ffi-design/<lib>/` |
...
- [ ] 所有文件无内部 reference 痕迹：`P1`-`P11`、`FFI-ANALYSIS`、`general-c-ffi`、`comparison`、`→ §`。
