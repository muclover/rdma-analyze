...
在 C 项目目录内收集。默认以公开头文件、构建元数据和官方文档作为 FFI 设计依据；只有为确认 ABI、宏/inline、错误模型、线程/回调行为或资源释放语义时，才按问题读取相关 `.c` 小片段。
...
- 不要通读 vendored `.c` 或重写 C API 手册。默认以公开头文件、构建元数据和官方文档作为 FFI 设计依据；只有为确认 ABI、宏/inline、错误模型、线程/回调行为或资源释放语义时，才按问题读取相关 `.c` 小片段，并在设计中避免依赖未公开实现细节。
- 不要把 C++ 全量绑定纳入本技能范围。
...
- 内部参考索引：[references/README.md](references/README.md)
- 输出模版索引：[templates/README.md](templates/README.md)
