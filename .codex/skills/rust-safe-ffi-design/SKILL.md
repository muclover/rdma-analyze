...
Reference 是内部决策材料，不是交付物内容。读取要按需、分层，避免把 evidence 痕迹带入最终文档。

`core/` 是每次必读的干净执行手册，可转化为交付物判断。`evidence/` 是内部样本和横向材料，只能辅助判断，不能复制进交付物。
...
| 画像触发 | 可读 reference |
|----------|----------------|
| 纯 `-sys`、复杂 `build.rs` / system-vendored 切换 | `references/evidence/cases/libz-sys.md` |
| 双 crate、回调、panic/线程边界 | `references/evidence/cases/curl-rust.md` |
| workspace、错误栈、手写+bindgen 混合 | `references/evidence/cases/rust-openssl.md` |
| 三层、预生成 bindgen、大 API 面 | `references/evidence/cases/zstd-rs.md` |
| ibverbs / rdma-core / RDMA 相关 | `references/evidence/rdma/README.md` 与 `references/evidence/rdma/rdma-overview.md` |
| 需要横向模式核对 | `references/evidence/comparison/general-c-ffi.md` |
...
- 内部 evidence：[references/evidence/](references/evidence/)
- 副本同步说明：[references/maintenance/SOURCE.md](references/maintenance/SOURCE.md)
