...
> 注：本仓库未 vendor 该项目；下列结论来自其公开的 `Cargo.toml` / `build.rs` 与 README。

---

## 方案定位

`ruapc-rdma-sys` 走的是 **「最小够用表面」**：仍 **直连系统 `libibverbs`**（pkg-config + 动态链接），仍用 **bindgen**，但通过 **`allowlist_*` 极度收窄**生成体积，并在生成后对 **`bindings.rs` 做 Rust AST 级改写**，把若干字段从「裸整数数组」升级为 **`FwVer`、`Guid`、`WRID`、`LinkLayer`** 等新类型，以及对 **`ibv_device_attr` / `ibv_port_attr`** 等施加 **`Serialize` / `Deserialize` / `JsonSchema`**。

**它不是「两层 FFI」**（没有 mummy、也没有单独 C wrapper crate）：**绑定层即产品的 sys 层**，上层 ruapc 工程在此之上做自己的协议与控制平面。
...
```text
  ┌─────────────────────────────────────────────────────────────┐
  │           ruapc 主工程其它 crate（控制平面 / 工具链）           │
  │   消费 ruapc-rdma-sys 的类型与少量 verbs 函数                  │
  └────────────────────────────┬────────────────────────────────┘
                               │
                               ▼
  ┌─────────────────────────────────────────────────────────────┐
  ...
  │       ↓                                                       │
  │  bindgen（窄 allowlist）→ 字符串形式的 bindings               │
  │       ↓                                                       │
  │  syn parse → 替换字段类型 → prettyplease → OUT_DIR/bindings.rs │
  └────────────────────────────┬────────────────────────────────┘
                               │ 动态链接
                               ▼
                        libibverbs.so
...
**与其它四条路线的本质区别：**  
不把「全量 verbs」作为目标；把 **「运维友好 / 序列化友好 / CLI 友好」**写进 **绑定生成管线**（derive + AST 手术），而不是单独再做一个大型的 **`sideway`/`async-rdma` 风格 safe verbs** crate。
...
| 机制 | 目的 |
|------|------|
| **窄 allowlist** | 只绑定 ruapc **实际用到的** device/CQ/QP/MR/poll/post/query 等；缩短编译时间、缩小维护面 |
| **`ParseCallbacks::add_derives`** | 对选定结构自动 **`Serialize` / `Deserialize` / `JsonSchema`**，服务配置与观测 |
| **`opaque_type` / `no_copy` / `no_debug`** | 屏蔽 pthread、函数指针字段等 **不适合自动 derive / Copy** 的成员 |
| **`syn` + `prettyplease` 后处理** | 在生成码上 **精确替换字段类型**（如固件版本、GUID），避免手写重复绑定 |

**inline / `ibv_post_send`：** allowlist 包含这些符号时，bindgen 产出 **`extern "C"`**。能否链接取决于 **`libibverbs.so` 是否导出对应符号**（主线 rdma-core 上通常可行）。**若某平台链接失败**，需要回到「Rust ops 派发」或「薄 C wrapper」等兜底——这不是 ruapc 当前文档强调的路径，但是架构上可能的退路。

---

## 安全 Rust 封装层：是否存在「第二层」？

**严格说：** `ruapc-rdma-sys` **本身就是底层 crate**，README 侧重 **device 管理与类型辅助**，**不是** 完整的 ibverbs 安全封装框架。

**上层封装在哪：** 在 **ruapc 仓库其它 crate**（具体命名以仓库为准）。它们可能提供：

- **CLI / 配置导入导出**（受益于 serde/schema）；
- **设备枚举与健康检查逻辑**；
- **与 ruapc 协议栈其它部分的集成**。

**要点：** 与本目录其它专题中的 **`ibverbs` / `sideway` / `async-rdma`** 相比，**ruapc 没有在开源边界内提供一个「通用 verbs 教程式 safe 层」**；**价值集中在「窄 FFI + 强类型字段 + 序列化」**。
...
**同一层的 sys-style FFI（直连系统 libibverbs），特色是「窄绑定 + bindgen 后 AST 手术 + serde/schema」，而非两层 FFI、mummy 或 C wrapper。** 上层 ruapc **另有 crate** 承接业务与工具链，**但不等价于通用 verbs 安全封装库**。
